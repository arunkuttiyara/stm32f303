...
/* global varaible decalarations*/
extern __IO uint32_t uwTick;
int sec=0;      // used to keep a track of seconds
int16_t xyz[3]; // to store accelerometer values
int alarm=0;    // alarm flag used to denote alarm stage.
...
int flash_LED()
{
	...
	/***********************      generating pattern for led in normal condition   	*************/

	sec++;
	if(alarm == ALARM_OFF) //turning off and on leds alternativily 
	{	
		if (sec%8 == 0)       
		{
			BSP_LED_Off(LED4);
	...
	else 
	{
		...
		if(sec%2==0)
		{
			BSP_LED_On(LED_RED);     //flashing the leds
			BSP_LED_On(LED5);
			BSP_LED_On(LED7);
			BSP_LED_On(LED9);
			BSP_LED_On(LED10);
...
void HAL_IncTick(void)
{
	uwTick++; 		 
	if((uwTick%1000) ==0)
		ready_flag=1; 		 // the delay has reached 1 sec, which means led functions can be processed
...
int main(int argc, char **argv)
{
	...
	while(1)
	{
		...
		if(xyz[0] > 1000)	         //if x value of accelerometer cross 1000 alaram value is set to '1'
			alarm=ALARM_ON;		 // ALARM !!

		if(BSP_PB_GetState(BUTTON_USER) && alarm) //when push button is pressed all leds goes "off"
		{    
			alarm=ALARM_OFF;
			BSP_LED_Off(LED_RED);
			BSP_LED_Off(LED5);
