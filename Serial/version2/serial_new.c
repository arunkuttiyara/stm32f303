...
/* flag used to communicate with main */
extern int int_ok;
...
{
	/*LED 4 is made ON to show the recieve call back is working */
	BSP_LED_On(4);	
	/* changing the flag status to denote the interrupt generation */
	int_ok=1;
}
...
void HAL_UART_TxCpltCallback(UART_HandleTypeDef *huart)
{
	...
	/* LED 3 is made ON to denote the staus of a successful transmission */
	BSP_LED_On(6);	
}
...
{
	/* LED 5 is on if call back error is occured */
	BSP_LED_On(5);	
}
// IRQ Handlers
...
ADD_CMD("uart",UART1_DMA1_Init,"uart  initialization ")
